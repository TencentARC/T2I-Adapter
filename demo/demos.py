    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                cond_strength = gr.Slider(label="Controling Strength (The guidance strength of the keypose to the result)", minimum=0, maximum=1, value=1, step=0.1)
        ...
        ips = [input_img, type_in, prompt, neg_prompt, pos_prompt, fix_sample, scale, cond_strength, base_model]
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                run_button = gr.Button(label="Run")
                cond_strength = gr.Slider(label="Controling Strength (The guidance strength of the sketch to the result)", minimum=0, maximum=1, value=0.4, step=0.1)
                scale = gr.Slider(label="Guidance Scale (Classifier free guidance)", minimum=0.1, maximum=30.0, value=9, step=0.1)
                fix_sample = gr.inputs.Radio(['True', 'False'], type="value", default='False', label='Fix Sampling\n (Fix the random seed)')
                base_model = gr.inputs.Radio(['sd-v1-4.ckpt', 'anything-v4.0-pruned.ckpt'], type="value", default='sd-v1-4.ckpt', label='The base model you want to use')
            with gr.Column():
                result = gr.Gallery(label='Output', show_label=False, elem_id="gallery").style(grid=2, height='auto')
            ips = [input_img, type_in, color_back, prompt, neg_prompt, pos_prompt, fix_sample, scale, cond_strength, base_model]
        run_button.click(fn=process, inputs=ips, outputs=[result])
    return demo
...
def create_demo_draw(process):
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
                result = gr.Gallery(label='Output', show_label=False, elem_id="gallery").style(grid=2, height='auto')
            ips = [input_img, prompt, neg_prompt, pos_prompt, fix_sample, scale, cond_strength, base_model]
        run_button.click(fn=process, inputs=ips, outputs=[result])
    return demo