...
parser.add_argument(
        ...
        type=str,
        nargs="?",
        default="An all white kitchen with an electric stovetop"
    )
parser.add_argument(
        "--path_cond",
        type=str,
        default="examples/seg_sketch/mask.png"
)
parser.add_argument(
        "--path_cond2",
        type=str,
        default="examples/seg_sketch/edge.png"
)
parser.add_argument(
    "--bsize",
    type=int,
...
parser.add_argument(
        "--config",
        type=str,
        default="configs/stable-diffusion/test_mask_sketch.yaml",
...
if __name__ == '__main__':
    ...
    model_ad2 = Adapter(channels=[320, 640, 1280, 1280][:4], nums_rb=2, ksize=1, sk=True, use_conv=False).to(device)

    # to gpus
    model_ad = torch.nn.parallel.DistributedDataParallel(
        model_ad,
        device_ids=[torch.cuda.current_device()])
    model_ad.module.load_state_dict(torch.load(opt.ckpt_ad2))
    model_ad2 = torch.nn.parallel.DistributedDataParallel(
        model_ad2,
        device_ids=[torch.cuda.current_device()])
    model_ad2.module.load_state_dict(torch.load(opt.ckpt_ad1))
    ...
    for v_idx in range(opt.n_samples):
        with torch.no_grad():
            ...
            edge = cv2.imread(opt.path_cond2)
            ...
            cv2.imwrite(os.path.join(experiments_root, 'visualization', 'mask_idx%04d.png'%(v_idx)), im_mask)
            im_edge = tensor2img(edge)
            cv2.imwrite(os.path.join(experiments_root, 'visualization', 'edge_idx%04d.png'%(v_idx)), im_edge)

            features_adapter1 = model_ad2(edge)
            features_adapter2 = model_ad(mask)
            ...
            samples_ddim, intermediates = sampler.sample(S=opt.ddim_steps,
                                                ...
                                                eta=opt.ddim_eta,
                                                x_T=None,
                                                features_adapter1=features_adapter1,
                                                features_adapter2=features_adapter2,
                                                mode = 'mul'
                                                )
