...
class OpenposeInference(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        body_modelpath = os.path.join('checkpoints', "body_pose_model.pth")

        if not os.path.exists(body_modelpath):
            from basicsr.utils.download_util import load_file_from_url
            load_file_from_url(remote_model_path, model_dir='checkpoints')
...
class OpenposeInference_count(nn.Module):
